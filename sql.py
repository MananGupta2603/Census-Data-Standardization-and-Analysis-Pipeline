...
def window_1():
    window('Total population of each District',"SELECT District_Code, State_UT, District, Population FROM Census_DB.Census")

# Window 2 : Literate male and female in each district

def window_2():
    window('Literate Male and Female in each district', \
            "SELECT District_Code, State_UT, District, Literate_Male, Literate_Female FROM Census_DB.Census")
...
def window_3():
    window('Percentage of Workers in each district',
            "SELECT District_Code, State_UT, District, ROUND((Workers/Population)*100, 2) \
                            AS Percentage_of_Workers FROM Census_DB.Census")
...
def window_4():
    window('Households with LPG or PNG as Cooking fuel',
            "SELECT District_Code, State_UT, District, LPG_or_PNG_Households FROM Census_DB.Census")
...
def window_6():
    window("Households with Internet in each District", 
            "SELECT District_Code, State_UT, District, Households_with_Internet FROM Census_DB.Census")
...
def window_7():
    window('Educational attainment distribution in each district',
            'SELECT District_Code, State_UT, District, Below_Primary_Education, Primary_Education, Middle_Education, \
                ...
                Illiterate_Education, Total_Education FROM Census_DB.Census')
...
def window_8():
    window('Households with access to various modes of transports in each district', 
            'SELECT District_Code, State_UT, District, Households_with_Bicycle, Households_with_Car_Jeep_Van, \
                Households_with_Radio_Transistor, Households_with_Scooter_Motorcycle_Moped FROM Census_DB.Census')
...
def window_19():
    window('Households below the poverty line in each state',
            'SELECT State_UT, SUM(Power_Parity_Less_than_Rs_45000) AS Households_below_poverty_line FROM Census_DB.Census \
            GROUP BY State_UT')
...
def main():
    st.title("Census Data Cleaning and Processing Project")
    st.divider()
    options = ['Missing Data Visualization', '1) Total population of each District', '2) Literate Male and Female in each district',
                '3) Percentage of Workers in each district', '4) Households with LPG or PNG as Cooking fuel',
                '5) Religious composition of each district (in percentage)', '6) Households with Internet in each District',
                '7) Educational attainment distribution in each district',
                '8) Households with access to various modes of transports in each district', '9) Condition of occupied census houses',
                '10) Distribution of Household size', '11) Total number of households in each state', 
                '12) Households having Latrine facility within the premises in each state', '13) Average household size in each state',
                '14) Owned versus rented houses in each state', '15) Distribution of different types of latrine facilities in each state',
                '16) Houeholds with drinking water sources near the premises in each state',
                '17) Average household income distribution in each state based on the power parity categories',
                '18) Percentage of married couples with different household sizes in each state',
                '19) Households below the poverty line in each state', '20) Overall literacy rate of each state']

    select = st.selectbox("",options = options, index = None, placeholder = 'Choose an option')
    if select == 'Missing Data Visualization':
        missing_data('Original_Data\census-2011.csv')
    elif select == '1) Total population of each District':
        window_1()
    elif select == '2) Literate Male and Female in each district':
        window_2()
    elif select == '3) Percentage of Workers in each district':
        window_3()
    elif select == '4) Households with LPG or PNG as Cooking fuel':
        window_4()
    elif select == '5) Religious composition of each district (in percentage)':
        window_5()
    elif select == '6) Households with Internet in each District':
        window_6()
    elif select == '7) Educational attainment distribution in each district':
        window_7()
    elif select == '8) Households with access to various modes of transports in each district':
        window_8()
    elif select == '9) Condition of occupied census houses':
        window_9()
    elif select == '10) Distribution of Household size':
        window_10()
    elif select == '11) Total number of households in each state':
        window_11()
    elif select == '12) Households having Latrine facility within the premises in each state':
        window_12()
    elif select == '13) Average household size in each state':
        window_13()
    elif select == '14) Owned versus rented houses in each state':
        window_14()
    elif select == '15) Distribution of different types of latrine facilities in each state':
        window_15()
    elif select == '16) Houeholds with drinking water sources near the premises in each state':
        window_16()
    elif select == '17) Average household income distribution in each state based on the power parity categories':
        window_17()
    elif select == '18) Percentage of married couples with different household sizes in each state':
        window_18()
    elif select == '19) Households below the poverty line in each state':
        window_19()
    elif select == '20) Overall literacy rate of each state':
        window_20()
    else:
        st.write("Please Select any options")
